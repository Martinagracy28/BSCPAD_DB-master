...
class fourthpage extends React.Component {
  constructor(props) {
      ...
      //this.state = {Con1: []};
       ...
       check() {
        let ref2=fireDB.database().ref(`demo`);
        const db = ref2.push().key;   
        ref2.child(db).set({id:"test"})
                      .then(()=>{});
      }
      componentDidMount() {
        ...
        fireDB.database().ref("demo").on("value", (data) => {
           if (data) {
            console.log("dtat",data)
            data.forEach((d) => {
              let value=d.val();
              console.log("keycheckCollection",value)
      ...
      let g = fireDB.database().ref("1");
      console.log("goal",g._delegate._path)
      fireDB.database().ref("1").orderByChild("goal").equalTo(1000000).on("value", snapshot => {
       let Con = [];
